...
public class cls_funcionesM2 {
    ...
    }//fin llenarcbxdeportes

    //----------------------METODOS M2P3----------------------
    public void crearRutinas(String nombreRutina, String deporteAsignado, String descripcion, String duracion, boolean status) {
        //Intancia
        cls_rutina R = new cls_rutina();
        R.setNombreRutina(nombreRutina);
        R.setDeporteAsignado(deporteAsignado);
    ...
    public String mostrarRutina() {
        ...
        return s;
    }//fin metodo mostrar Rutina

    public void desactivarRutina(String nombre) {

        boolean desactivado = false; // solo se activa si el deporte fue encontrado y desactivado || se utiliza para mensaje
        boolean encontrado = false;
        //---LISTA RUTINAS--
        for (int x = 0; x < listaRutinas.size(); x++) {
            if (listaRutinas.get(x).getNombreRutina().equals(nombre)) {
                encontrado = true;
